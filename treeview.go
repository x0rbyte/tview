...
func (t *TreeView) process(drawingAfter bool) {
	...
	if selectedIndex >= 0 {
		...
		// Ensure selectedIndex is within bounds before accessing
		if selectedIndex < 0 || selectedIndex >= len(t.nodes) {
			// Selection is out of bounds, keep current node but don't update index
			// This prevents the crash while maintaining tree state
		} else {
